"use client"
// ═══════════════════════════════════════════════
// SOCIAL MEDIA SECTION — Fixed: gradient via inline style
...
// SVG brand icons as components (no external library needed)
...
export function Social() {
  ...
  return (
    <section className="py-24 px-6 w-full max-w-5xl mx-auto relative z-20">
      ...
      >
        <p className="text-sm font-mono text-primary/70 mb-1">{t.social.subtitle}</p>
        <h2 className="text-4xl md:text-5xl font-black text-foreground mb-4">
          {t.social.title}
        </h2>
        <div className="h-1 w-24 bg-gradient-to-r from-pink-400 to-purple-400 rounded-full mb-6" />
        <p className="text-lg text-foreground/70 max-w-xl">{t.social.description}</p>
      </motion.div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
        {PORTFOLIO_CONFIG.socials.map((social, idx) => {
          const Icon = IconComponents[social.id] || IconComponents.github
          ...
          return (
            ...
            >
              ...
              <div
                ...
                style={{
                  background: `linear-gradient(135deg, ${social.color}dd, ${social.color}88)`,
                }}
              />
              {/* Glow circle deco */}
              <div
                className="absolute -top-6 -right-6 w-24 h-24 rounded-full blur-2xl opacity-30 group-hover:opacity-60 transition-opacity duration-500"
              ...
              <span className="relative z-10 font-black text-lg text-foreground/90 group-hover:text-white transition-colors duration-400">
                {social.label}
              </span>
            </motion.a>
          )
